gvMapText = ""..
		...
		" @cr @cr @cr @color:150,0,255 Ghoul @color:230,0,240 @cr (3) Marys Verrat "
gvMapVersion = " v1.2 "
...
function TributePaid_P1_Easy()
	...
	--
	MultiplayerTools.GiveBuyableHerosToHumanPlayer( 7 )
	gvDiffLVL = 2.0
...
function StartInitialize()
	...
	local i
	for i=1,8,1 do
       Tools.GiveResouces(i, dekaround(800*gvDiffLVL) , dekaround(1000*gvDiffLVL), dekaround(1000*gvDiffLVL), dekaround(700*gvDiffLVL), 0, 0)
    end
	--
	CreateArmy1()

	MapEditor_SetupAI(4,1,99999,1,"P4",1,0)
	MapEditor_SetupAI(5,3,99999,1,"P5",3,0)
	MapEditor_SetupAI(6,3,99999,1,"P6",3,0)
	MapEditor_SetupAI(7,3,99999,1,"P7",3,0)
	MapEditor_SetupAI(8,3,99999,1,"P8",3,0)
	for i = 5,8 do
		MapEditor_Armies[i].offensiveArmies.strength = round(25/gvDiffLVL)
	end
	--
...
function UpgradeKIb()
	Message("Obacht Herr! @cr Die Armeenstärke Eures Feindes ist gestiegen!")
	for i = 4,8 do
		Logic.UpgradeSettlerCategory(UpgradeCategories.LeaderPoleArm,i)
		Logic.UpgradeSettlerCategory(UpgradeCategories.SoldierPoleArm,i)
		Logic.UpgradeSettlerCategory(UpgradeCategories.LeaderBow,i)
	...
	StartCountdown(100*60*gvDiffLVL,UpgradeKIc,false)
end
function UpgradeKIc()
	Message("Achtung Herr! @cr Der Feind verfügt nun über die besten Technologien. Außerdem ist seine Armeenstärke gestiegen. @cr Es wird nicht leicht werden, ihn in einer Schlacht zu besiegen!")
	for i = 4,8 do
...
function CreateArmy1()
	...
	army1.id				= 1
	army1.strength		= 3
	army1.position		= GetPosition("P4Army1")
...
function CreateArmyTwo()
	...
	armyTwo.id				= 2
	armyTwo.strength		= 2
	armyTwo.position		= GetPosition("P4Army1")
...
function ControlArmy1()
    ...
    if IsDead(army1) and IsExisting("P4Turm") then
...
function ControlArmyTwo()
	...
	if IsDead(armyTwo) and IsExisting("P4Turm") then
...
function Sieg()
	if IsDestroyed("P4Barracks") and IsDestroyed("P5HQ") and IsDestroyed("P6HQ") and IsDestroyed("P7HQ") and IsDestroyed("P8HQ") and IsDestroyed("MaryHQ") then
