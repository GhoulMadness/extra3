...
function Start()
	...
	do
		...
		army.id = GetFirstFreeArmySlot(3)
		army.player = 3
		army.position = GetPosition("KerbSpawn")
		army.strength = 12 - 3 * gvDiffLVL
		army.rodeLength = 7700 - (700 * gvDiffLVL)
		army.spawnTypes = {{Entities.CU_VeteranCaptain, 0}, {Entities.PU_LeaderBow4, 12}, {Entities.CU_VeteranMajor, 2}, {Entities.CU_BlackKnight_LeaderSword3, 6}}
		army.spawnPos = GetPosition("KerbSpawn")
		army.spawnGenerator = GetID("KerbTower")
		army.respawnTime = round(40*gvDiffLVL)
		army.maxSpawnAmount = 1
		army.endless = true
		SetupArmy(army)
		EnlargeArmy(army, {leaderType = army.spawnTypes[1][1]})
		SetupAITroopSpawnGenerator("MixedKerbArmy", army)
		army.trigger = Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND,"", "ControlBanditArmy",1,{},{army.player, army.id})
	end
	IntroNotes()
end
...
function Kroxon_Fallen()
	if not IsExisting("Burg2") then
		...
		briefing.finished = function()
			...
			StartSimpleJob("CheckForDarioNearOutpostRuin")
		end
		StartBriefing(briefing)
		return true
	end
end
function CheckForDarioNearOutpostRuin()
	local pos = GetPosition("OutpostRuin")
	if IsNear("Dario", "OutpostRuin", 700)
	and not AreEntitiesOfCategoriesAndDiplomacyStateInArea(1, {EntityCategories.Leader, EntityCategories.Soldier, EntityCategories.Cannon}, pos, 3000, Diplomacy.Hostile) then
		...
		briefing.finished = function()
			...
			Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND,"", "InitRuinRepairing",1,{},{"outpost_repair","OutpostRuin","OutpostRuin_rep1","OutpostRuin_rep2","OutpostRuin_rep3","OutpostRuin_rep4", Entities.PB_Outpost1, round(240*2/gvDiffLVL), 1200, 6})
			StartSimpleJob("Outpost_Done")
		end
		StartBriefing(briefing)
		return true
	end
end
function Outpost_Done()
	local pos = GetPosition("OutpostRuin")
...
function Afraid_Miner()
    ...
    Callback = function()
		...
		briefing.finished = function()
			...
			Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND,"", "InitRuinRepairing",1,{},{"residence_repair","ruin_residence","ruin_residence_rep1","ruin_residence_rep2","ruin_residence_rep3","ruin_residence_rep4", Entities.PB_Residence3, round(80*2/gvDiffLVL), 1000, 6})
			Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND,"", "InitRuinRepairing",1,{},{"claymine_repair","ruin_claymine","ruin_claymine_rep1","ruin_claymine_rep2","ruin_claymine_rep3","ruin_claymine_rep4", Entities.PB_ClayMine3, round(120*2/gvDiffLVL), 1200, 6})
			Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND,"", "InitRuinRepairing",1,{},{"farm_repair","ruin_farm","ruin_farm_rep1","ruin_farm_rep2","ruin_farm_rep3","ruin_farm_rep4", Entities.PB_Farm3, round(80*2/gvDiffLVL), 1000, 6})
		end
		StartBriefing(briefing)
	end}
