...
function FirstMapAction()
	...
	--
	table.insert(ChestRandomPositions.ForbiddenSectors, 21)
	table.insert(ChestRandomPositions.ForbiddenSectors, 82)
	table.insert(ChestRandomPositions.ForbiddenSectors, 132)
end
...
function Prolog()
	...
	AP{
		title = dario,
		text = "Wir werden diese brennende Ruine für den Bau unserer Siedlung benutzen.",
		position = GetPosition("Sicht"),
		dialogCamera = false,
		action = function()
			Camera.RotSetAngle(180)
			Camera.RotSetFlipBack(0)
		end
	}
	AP{
		title = dario,
		text = "Meine Getreuen, reparieren wir zunächst die Burg. @cr Dann suchen wir nach Kerberos.",
...
function CheckForDarioNearOutpostRuinVarg()
	...
	and not AreEntitiesOfCategoriesAndDiplomacyStateInArea(1, {EntityCategories.Leader, EntityCategories.Soldier, EntityCategories.Cannon}, pos, 3000, Diplomacy.Hostile) then
		...
		briefing.finished = function()
			Camera.RotSetAngle(-45)
			Camera.RotSetFlipBack(1)
			Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND,"", "InitRuinRepairing",1,{},{"outpost_repair_varg","OutpostRuinVarg","OutpostRuinVarg_rep1","OutpostRuinVarg_rep2","OutpostRuinVarg_rep3","OutpostRuinVarg_rep4", Entities.PB_Outpost1, round(240*2/gvDiffLVL), 1200, 6})
...
function OutpostVarg_Done()
	local pos = GetPosition("OutpostRuinVarg")
	local op = {Logic.GetPlayerEntitiesInArea(6,Entities.PB_Outpost1,pos.X,pos.Y,1000,1)}
...
function Dovbar_1()
    ...
    Callback = function()
		...
		briefing.finished = function()
			OUTPOSTMARKERPOS = GetPosition("KerberosBurg")
...
function Johannes()
	local BeiJo = {
		...
		Callback = function()
			...
			briefing.finished = function()
				PRISONMARKERPOS = GetPosition("Knast")
...
function Priester()
	local BeiPr = {
		...
		Callback = function()
			...
			briefing.finished = function()
				GAREKMARKERPOS = GetPosition("Garek")
...
function Dovbar_2()
	local BeiDov = {
		...
		Callback = function()
			...
			briefing.finished = function()
				SULFURMARKERPOS = GetPosition("Schwefel")
				EnableNpcMarker(GetEntityId("Pilgrim"))
				Pilgrim_2()
...
function Gebaut()
	if gvMi == 1 then
		...
		briefing.finished = function()
			...
			StartSimpleJob("Nah")
			Abbau()
			GUI.DestroyMinimapPulse(SULFURMARKERPOS.X, SULFURMARKERPOS.Y)
		end;
		StartBriefing(briefing);
		return true
	end
	...
	local BeiPilgr = {
		...
		Callback = function()
			...
			briefing.finished = function()
				...
				--
				StartCountdown(30*60/gvDiffLVL, DrakonSupply, false)
...
function DrakonSupply()
	local trooptypes = {Entities.PV_Cannon3, Entities.PV_Cannon5, Entities.PU_LeaderRifle2, Entities.PU_LeaderPoleArm4}
	local army 		= {}
	army.player 	= 1
	army.id 		= GetFirstFreeArmySlot(1)
	army.position 	= GetPosition("DrakonSupport")
	army.rodeLength	= 0
	army.strength	= round(2+(2*gvDiffLVL))
	SetupArmy(army)
	for i = 1, army.strength do
		EnlargeArmy(army, {leaderType = trooptypes[math.random(table.getn(trooptypes))]})
	end
	local briefing = {}
	local AP, ASP = AddPages(briefing)
	ASP("DrakonSupport",ment,"Sehr gute Nachrichten Sire! @cr Verstärkungstruppen aus Drakon sind soeben eingetroffen und erwarten Eure Befehle.", false)
	briefing.finished = function()
	end;
	StartBriefing(briefing);
end
...
function Garek()
	local Beiga = {
		...
		Callback = function()
			...
			AP{
				title = ga,
				text = "Aber wenn ihr uns unbedingt helfen wollt, könnt ihr für uns die Kirchenruine dort hinten abreißen und dafür an dieser Stelle eine prunkvolle neue Kirche bauen.",
				position = GetPosition("Ruine"),
				marker = STATIC_MARKER,
				dialogCamera = false,
			}
			...
			briefing.finished = function()
				CHAPELRUINMARKERPOS = GetPosition("Gate")
				Logic.RemoveQuest(1,GarekQuest)
				GUI.DestroyMinimapPulse(GAREKMARKERPOS.X, GAREKMARKERPOS.Y)
				Gate()
				RuinQuest()
				Vorbereitung_3()
...
function Errichtet()
	if gvKi == 1 then
    	...
    	briefing.finished = function()
			...
			EnableNpcMarker(GetEntityId("Johannes"))
			JohannesQuest_3()
			GUI.DestroyMinimapPulse(CHAPELRUINMARKERPOS.X, CHAPELRUINMARKERPOS.Y)
		end;
		StartBriefing(briefing);
		return true
	end
	...
	local BeiBi = {
		...
		Callback = function()
			...
			briefing.finished = function()
				...
				GUI.DestroyMinimapPulse(PRISONMARKERPOS.X, PRISONMARKERPOS.Y)
				BishopQuest()
...
function MonksFreedJob()
	if IsDestroyed("prison") then
		...
		briefing.finished = function()
			LASTQUESTMARKERPOS = GetPosition("Pfad")
...
function Fertig()
	if IsDestroyed("KerberosBurg") then
		...
		briefing.finished = function()
			...
			GUI.DestroyMinimapPulse(OUTPOSTMARKERPOS.X, OUTPOSTMARKERPOS.Y)
			Tor()
...
function Gewonnen()
	if IsNear("Dario","Pfad",900) then
		GUI.DestroyMinimapPulse(LASTQUESTMARKERPOS.X, LASTQUESTMARKERPOS.Y)
