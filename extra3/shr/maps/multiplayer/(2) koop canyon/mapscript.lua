...
gvMapText = ""..
		...
		" @cr @cr @cr @color:150,0,255 Ghoul @color:230,0,240 @cr (2) Canyon "
gvMapVersion = " v1.4 "
...
function FirstMapAction()
	...
	-- Spieler 3 und 5 sowie 4 und 6 gleiche Farben
...
function StartInitialize()
	...
	MapEditor_SetupAI(3, 1, 25000, 1, "P3_HQ", 3, 900*gvDiffLVL)
	SetupPlayerAi(3, {
	serfLimit = 16,
	extracting = 1,
	constructing = true,
...
function UpgradeAITroops4()
	for i = 3,4 do
		...
		MapEditor_Armies[i].offensiveArmies.strength				=	30
		MapEditor_Armies[i].defensiveArmies.strength				=	8
	end
...
function UpgradeAITroops6()
	for i = 3,4 do
		...
		MapEditor_Armies[i].offensiveArmies.strength				=	45
		MapEditor_Armies[i].defensiveArmies.strength				=	12
	end
...
function VictoryJob()
	if IsDead("P3_HQ") and IsDead("P4_HQ") then
...
function AnfangsBriefing()	--# Intro, noch fast leer
    ...
    AP{
        ...
        text	= "@color:20,255,50 Ihr seid hier aber nicht alleine. Zwei ferne Städte sind Fremden nicht gerade freundlich aufgeschlossen. @cr @color:150,150,150 (Weiter mit Esc)"
    }
    AP{
        title	= "@color:230,0,0 Intro",
...
function SetupExtraArmys()
	...
	for p = 5, 6 do
		...
		SetupPlayerAi(p, {serfLimit = 0, extracting = 0, constructing = false, repairing = false})

		ExtraArmy[p] = {}
		ExtraArmy[p].strength		= 8
		ExtraArmy[p].id          	= 1
		ExtraArmy[p].player			= p
		ExtraArmy[p].position		= GetPosition("posP"..p)
		ExtraArmy[p].rodeLength		= Logic.WorldGetSize()

		SetupArmy(ExtraArmy[p])
	...
	-- Truppen
		...
		troopDescriptionLow[8] = {maxNumberOfSoldiers = 0, leaderType = Entities.PV_Cannon1}

		-- mittel

		troopDescriptionNormal = {}

		troopDescriptionNormal[1] = {maxNumberOfSoldiers = 12, leaderType = Entities.PU_LeaderSword4}
		troopDescriptionNormal[2] = {maxNumberOfSoldiers = 12, leaderType = Entities.PU_LeaderSword4}

		troopDescriptionNormal[3] = {maxNumberOfSoldiers = 6, leaderType = Entities.PU_LeaderRifle2}
		troopDescriptionNormal[4] = {maxNumberOfSoldiers = 6, leaderType = Entities.PU_LeaderRifle2}
		...
		if time < 70*60 then
			...
			for p = 5, 6 do
				...
				for n = 1, table.getn(troopDescriptionLow) do
					...
					EnlargeArmy(ExtraArmy[p], troopDescriptionLow[n])
			...
			for p = 5, 6 do
				...
				for n = 1, table.getn(troopDescriptionNormal) do
					...
					EnlargeArmy(ExtraArmy[p], troopDescriptionNormal[n])
