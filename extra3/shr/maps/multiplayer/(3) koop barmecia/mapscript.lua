...
function ColeshillChestPlundered()
    ...
    AP{
		...
		action = function()
			...
			Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND,"","MakePlayerEntitiesInvulnerableLimitedTime",1,{},{8, 50 / gvDiffLVL})
		end
	}
...
function TroopSpawnVorb()
	...
	local TimeNeeded = math.floor(math.min((1.8+((math.random(1,5)/10)) + (timeb4end/120)) * 60 * gvDiffLVL, 8.5*60/(math.sqrt(gvDiffLVL))))
