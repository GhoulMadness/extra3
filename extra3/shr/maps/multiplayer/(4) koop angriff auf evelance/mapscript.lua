...
function StartInitialize()
	MakeInvulnerable("p5_tower1")
	MakeInvulnerable("p5_tower2")
	MakeInvulnerable("p5_tower3")
	MakeInvulnerable("p5_tower4")
	MakeInvulnerable("p5_tower5")
	MakeInvulnerable("p5_tower6")
	MakeInvulnerable("p5_tower7")
	MakeInvulnerable("p5_tower8")
	for i = 1, 8 do
		local army = {}
		army.player 	= 5
		army.id 		= GetFirstFreeArmySlot(5)
		army.strength 	= round(6/gvDiffLVL)
		army.position 	= GetPosition("p5_spawn" .. i)
		army.troopType  = Entities.CU_BlackKnight_LeaderMace2
		army.building	= GetID("p5_tower" .. i)
		army.rodeLength = 6500
		SetupArmy(army)
		for j = 1, army.strength do
			EnlargeArmy(army, {leaderType = army.troopType})
		end
	...
	for i = 5, 8 do
		local army = {}
		army.player 	= i
		army.id 		= GetFirstFreeArmySlot(i)
		army.strength 	= round(8/gvDiffLVL)
		army.position 	= GetPosition("p" .. i .. "spawn")
		army.troopType 	= Entities.PU_LeaderBow4
		army.building 	= GetID("p" .. i .. "hq")
		army.rodeLength = 6800
		SetupArmy(army)
		for j = 1, army.strength do
			EnlargeArmy(army, {leaderType = army.troopType})
		end
...
function ControlArmies(_player, _id)
	...
	if IsDead(army) and IsDestroyed(army.building) then
		return true
	end
	if IsVeryWeak(army) and IsExisting(army.building) then
		if Counter.Tick2("RespawnArmy_" .. _player .. "_" .. _id .. "_" .. army.building .. "_Counter", round(45 * gvDiffLVL)) then
...
function PeacetimeOver()
	...
	MakeVulnerable("p5_tower1")
	MakeVulnerable("p5_tower2")
	MakeVulnerable("p5_tower3")
	MakeVulnerable("p5_tower4")
	MakeVulnerable("p5_tower5")
	MakeVulnerable("p5_tower6")
	MakeVulnerable("p5_tower7")
	MakeVulnerable("p5_tower8")
end
function ActivateKI()
	for i = 5,8 do
		MapEditor_SetupAI(i,2,Logic.WorldGetSize(),3,"P"..i,3,0)
		--
		MapEditor_Armies[i].offensiveArmies.strength = round(32/gvDiffLVL)
	end
end
...
function UpgradeKI()
	...
	StartCountdown(45*60*gvDiffLVL,UpgradeKIb,false)
end
function UpgradeKIb()
	for i = 5,8 do
