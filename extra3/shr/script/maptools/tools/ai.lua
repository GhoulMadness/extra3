...
MapEditor_Armies_RefreshHomeSector = function(_playerId, _rootEntity)
	if Counter.Tick2("MapEditor_Armies_RefreshHomeSector_Counter_" .. _playerId, 20) then
		local id = GetID(_rootEntity)
		if id == 0 then
			local pos = MapEditor_Armies[_playerId].defensiveArmies.position
			id = Logic.GetEntityAtPosition(pos.X, pos.Y)
		end
		if id > 0 then
			MapEditor_Armies[_playerId].Sector = Logic.GetSector(id)
		end
	end
end
			...
			if Logic.EntityGetPlayer(id) == _playerId and Logic.IsBuilding(id) == 1 and CUtil.GetEntityClass(id) ~= 7798844 then
				...
				elseif string.find(etypeName, "Mine") ~= nil then
					...
					if uCat > 0 then
						...
						if not name then
							name = etypeName .. string.format("%.2f", Logic.GetTime()) .. math.random(2^16)
							Logic.SetEntityName(id, name)
						end
						if not MapEditor_Armies[_playerId].RebuildBuildingData[name] then
							MapEditor_Armies[_playerId].RebuildBuildingData[name] = {initialX = posX, initialY = posY, rotation = rot, level = level}
						else
							level = MapEditor_Armies[_playerId].RebuildBuildingData[name].level
							posX, posY = MapEditor_Armies[_playerId].RebuildBuildingData[name].initialX, MapEditor_Armies[_playerId].RebuildBuildingData[name].initialY
						end
						Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND, "", "MapEditor_Armies_Rebuild", 1, {}, {_playerId, uCat, rot, level, posX, posY, delay, 1, 0, name})
					end
				...
				or string.find(etypeName, "Outpost") ~= nil then
					...
					if uCat > 0 then
						...
						Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND, "", "MapEditor_Armies_Rebuild", 1, {}, {_playerId, uCat, rot, level, posX, posY, delay, 2, 0, name})
					end
				else
					local uCat, rot, level, delay = CollectData(id, etype)
					if uCat > 0 then
						...
						Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND, "", "MapEditor_Armies_Rebuild", 1, {}, {_playerId, uCat, rot, level, posX, posY, delay, 0, 0, name})
					end
				end
			end
		end
...
MapEditor_Armies_Rebuild = function(_playerId, _uCat, _rotation, _level, _posX, _posY, _delay, _builtOnType, _iteration, _name)
	if Counter.Tick2("MapEditor_Armies_Rebuild_Counter_" .. _playerId .. "_" .. _uCat .. "_" .. _posX .. "_" .. _posY, _delay) then
		if Logic.GetNumberOfEntitiesOfTypeOfPlayer(_playerId, Entities.PU_Serf) == 0 then
			if GetNumberOfPlayerEntitiesInEntityCategory(_playerId, EntityCategories.Headquarters) == 0 then
				...
				Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND, "", "MapEditor_Armies_Rebuild", 1, {}, {_playerId, _uCat, _rotation, _level, _posX, _posY, _delay, _builtOnType, 0, _name})
				return true
			end
		end
		local x, y
		local sector = MapEditor_Armies[_playerId].Sector
		if _builtOnType == 0 then
			...
			if _iteration and _iteration > MapEditor_Armies[_playerId].description.rebuildData.MaxAttemptsPreferredPosition then
				...
				x, y = EvaluateNearestUnblockedAreaWithAdditionalChecks(_playerId, baseX, baseY, rectWidth, rectHeight, sector, 3000, true, true, true, false)
			else
				x, y = EvaluateNearestUnblockedAreaWithAdditionalChecks(_playerId, _posX, _posY, rectWidth, rectHeight, sector, 1000 * (1 + (_iteration or 0)), true, true, true, false)
			end
			if not x or not y then
				Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND, "", "MapEditor_Armies_Rebuild", 1, {}, {_playerId, _uCat, _rotation, _level, _posX, _posY, _delay, _builtOnType, _iteration + 1, _name})
				return true
			end
		-- is built on a mine?
		elseif _builtOnType == 1 then
			...
			or Logic.IsMapPositionExplored(_playerId, _posX, _posY) == 0
			or sector ~= EvaluateNearestUnblockedSector(_posX, _posY, 2000, 100) then
				Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND, "", "MapEditor_Armies_Rebuild", 1, {}, {_playerId, _uCat, _rotation, _level, _posX, _posY, _delay, _builtOnType, 0, _name})
				return true
			else
				x, y = _posX, _posY
		...
		if csite > 0 and _level > 0 then
			...
			if id then
				...
				Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND, "", "MapEditor_Armies_UpgradeForRebuildNeeded", 1, {}, {id, _level, _name})
			end
		end
		return true
	end
