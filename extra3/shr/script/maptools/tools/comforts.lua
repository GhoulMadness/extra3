...
function IsCannonType(_type)
	...
	or _type == Entities.PV_Cannon6_2 or _type == Entities.PV_Catapult or _type == Entities.PV_Ram then
		return true
	end
	return false
end
...
function GetPositionBlockingType(_x, _y)
	...
	if Logic.GetWeatherState() == 3 and CUtil.GetWaterHeight(round(_x/100), round(_y/100)) >= CUtil.GetTerrainNodeHeight(round(_x/100), round(_y/100)) then
		return 8
	else
...
-- gets nearest unblocked area near a possibly blocked position
---@param _player integer playerID
---@param _posX number positionX
---@param _posY number positionY
...
---@param _offset integer maximum search range near position
---@param _checkVision boolean should only positions within player vision be valid?
---@param _checkSector boolean should only positions with same sector than search pos be valid?
---@param _checkEnemies boolean should only positions with no enemies nearby be valid?
---@param _checkCoalmine boolean should coalmine placement logic be applied to position checks?
---@param _performanceMode boolean should spiral search logic be used instead of point to point? false by default
---@return number positionX
---@return number positionY
EvaluateNearestUnblockedAreaWithAdditionalChecks = function(_player, _posX, _posY, _rectWidth, _rectHeight, _sector, _offset, _checkVision, _checkSector, _checkEnemies, _checkCoalmine, _performanceMode)
	local xInM, yInM = round(_posX/100), round(_posY/100)

	local halfWidthInM = _rectWidth / 200
    local halfLengthInM = _rectHeight / 200
    ...
    if BlockingGrid.IsAreaUnblocked(xInM, yInM, halfWidthInM, halfLengthInM)
	and AdditionalPlacementChecks(_player, _posX, _posY, _rectWidth, _rectHeight, _sector, 100, _checkVision, _checkSector, _checkEnemies) then
        return _posX, _posY
    end
	...
	local posMinX, posMaxX, posMinY, posMaxY = 0 + halfWidthInM, maxX - halfWidthInM, 0 + halfLengthInM, maxX - halfLengthInM
	if not _performanceMode then
		--point to point iteration
		for dy = -_offset/100, _offset/100, 1 do
			for dx = -_offset/100, _offset/100, 1 do
				local checkX = xInM + dx
				local checkY = yInM + dy
				if checkX >= posMinX and checkY >= posMinY and checkX <= posMaxX and checkY <= posMaxY then

					if BlockingGrid.IsAreaUnblocked(checkX, checkY, halfWidthInM, halfLengthInM)
					and AdditionalPlacementChecks(_player, checkX*100, checkY*100, _rectWidth, _rectHeight, _sector, 100, _checkVision, _checkSector, _checkEnemies) then
						if _checkCoalmine then
							local rot = 0
							while rot < 360 do
								if gvCoal.Mine.PlacementCheck(checkX * 100, checkY * 100, rot) then
									return checkX, checkY, rot
								end
								rot = rot + 5
							end
						else
							local d = (checkX - xInM)^2 + (checkY - yInM)^2
		...
		if bestX then
			return bestX*100, bestY*100
		end
	else
		--spiral search
		local searchRadius = 1
		while searchRadius <= _offset/100 do
			for offsetX = -searchRadius, searchRadius do
				local checks = {
					{math.max(math.min(xInM + offsetX, maxX), 0), math.max(math.min(yInM + searchRadius, maxX), 0)},
					{math.max(math.min(xInM + offsetX, maxX), 0), math.max(math.min(yInM - searchRadius, maxX), 0)},
					{math.max(math.min(xInM + searchRadius, maxX), 0), math.max(math.min(yInM + offsetX, maxX), 0)},
					{math.max(math.min(xInM - searchRadius, maxX), 0), math.max(math.min(yInM + offsetX, maxX), 0)}
				}
				for _, pos in ipairs(checks) do
					if BlockingGrid.IsAreaUnblocked(pos[1], pos[2], halfWidthInM, halfLengthInM)
					and AdditionalPlacementChecks(_player, pos[1] * 100, pos[2] * 100, _rectWidth, _rectHeight, _sector, 100, _checkVision, _checkSector, _checkEnemies) then
						if _checkCoalmine then
							local rot = 0
							while rot < 360 do
								if gvCoal.Mine.PlacementCheck(pos[1] * 100, pos[2] * 100, rot) then
									return pos[1] * 100, pos[2] * 100, rot
								end
								rot = rot + 10
							end
						else
							return pos[1] * 100, pos[2] * 100
						end
					end
				end
			end
...
---@return number positionX
---@return number positionY
EvaluateNearestUnblockedAreaWithinVisionRangeOfPlayer_HighPrecisionLowEfficiency = function(_player, _posX, _posY, _rectWidth, _rectHeight, _sector, _offset, _step, _noterrpos)
	if _noterrpos == nil then
		_noterrpos = true
	end
	local xmax, ymax = Logic.WorldGetSize()
	...
	local function AdditionalPlacementChecks(_player, _posX, _posY, _rectWidth, _rectHeight, _sector, _step)
		if Logic.IsMapPositionExplored(_player, _posX, _posY) == 1
		and _sector == EvaluateNearestUnblockedSector(_posX, _posY, _rectWidth + _rectHeight, _step)
		and not AreEntitiesOfCategoriesAndDiplomacyStateInArea(_player, {EntityCategories.Leader, EntityCategories.Soldier, EntityCategories.Cannon, EntityCategories.Hero}, {X = _posX, Y = _posY}, BS.EnemyBuildBlockRange/2, Diplomacy.Hostile) then
			return true
		end
		return false
	end

	if isAreaFree(f, res, _posX, _posY, _rectWidth, _rectHeight)
	and AdditionalPlacementChecks(_player, _posX, _posY, _rectWidth, _rectHeight, _sector, _step) then
		return _posX, _posY
	end

	local bestX, bestY, minDist
	local sizeX = Logic.WorldGetSize()
	local posMinX, posMaxX, posMinY, posMaxY = 0 + _rectWidth/2, sizeX - _rectWidth/2, 0 + _rectHeight/2, sizeX - _rectHeight/2

	for dy = -_offset, _offset, _step do
		for dx = -_offset, _offset, _step do
			local checkX = math.max(math.min(_posX + dx, posMaxX), posMinX)
			local checkY = math.max(math.min(_posY + dy, posMaxY), posMinY)

			if isAreaFree(f, res, checkX, checkY, _rectWidth, _rectHeight)
			and AdditionalPlacementChecks(_player, checkX, checkY, _rectWidth, _rectHeight, _sector, _step) then
...
EvaluateNearestUnblockedSector = function(_posX, _posY, _offset, _step)
	...
	-- if given position is already unblocked
	local sector = CUtil.GetSector(round(_posX/100), round(_posY/100))
	if sector > 0 then
		return sector
	else
	...
	for y_ = _posY - _offset, _posY + _offset, _step do
		for x_ = _posX - _offset, _posX + _offset, _step do
			if y_ > 0 and x_ > 0 and x_ < xmax and y_ < ymax then
				...
				local sector = CUtil.GetSector(round(x_/100), round(y_/100))
				if sector > 0 then
					return sector
				end
			end
...
-- do not spawn chests in these sectors
ChestRandomPositions.ForbiddenSectors = {}
...
ChestRandomPositions.GetRandomPositions = function(_amount)
    ...
    while table.getn(postable) < _amount do
        ...
        for _, eID in entities do
            ...
            if IsPositionUnblocked(_X, _Y)
			and table_findvalue(ChestRandomPositions.ForbiddenSectors, CUtil.GetSector(round(_X/100), round(_Y/100))) == 0 then
