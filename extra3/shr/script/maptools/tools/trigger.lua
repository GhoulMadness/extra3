...
function OnAriTroopCreated()
	...
	if entityType == Entities.PU_Hero5_Outlaw then
		Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_TURN, "", "AriTroopsDelayedSoldierSpawn", 1, {}, {entityID})
	end
end
-- delayed spawn, so army tables can be recalculated...
function AriTroopsDelayedSoldierSpawn(_id)
	local player = Logic.EntityGetPlayer(_id)
	local posX, posY = Logic.GetEntityPosition(_id)
	for i = 1, gvHero5.AbilityProperties.Summon.NumSoldiersPerTroop do
		...
		Logic.LeaderGetOneSoldier(_id)
	end
	local IDs = {Logic.GetSoldiersAttachedToLeader(_id)}
	Trigger.RequestTrigger(Events.LOGIC_EVENT_ENTITY_DESTROYED, "", "OnAriTroopDied", 1, {}, {_id, unpack(IDs)})
	return true
end
