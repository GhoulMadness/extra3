--------------------------------------------------------------------------------
-- MapName: (2) Aufstieg des Bösen - Kerberos dunkle Horden
--
-- Author: Ghoul
--
--------------------------------------------------------------------------------
...
--
	...
	CreateWoodPile("Holz1",1000000)
	CreateWoodPile("Holz2",1000000)
...
function AddMainquestForPlayer1()
	Logic.AddQuest(1,1,MAINQUEST_OPEN,"Missionsziele","Baut Eure Siedlungen auf und verteidigt sie gegen mögliche Angriffe! @cr @cr Besiegt Kerberos! @cr @cr @color:230,100,0 Spieler 1 kann den Spielmodus über das Tributmenü bestimmen!",1)
end

function AddMainquestForPlayer2()
	Logic.AddQuest(2,2,MAINQUEST_OPEN,"Missionsziele","Baut Eure Siedlungen auf und verteidigt sie gegen mögliche Angriffe! @cr @cr Besiegt Kerberos! @cr @cr @color:230,100,0 Spieler 1 kann den Spielmodus über das Tributmenü bestimmen!",1)
end
...
function KI2Wechsel()
	...
	AddGold(2,100000)
	MapEditor_SetupAI(2,3,100000,3,"P9",3,0)
	SetupPlayerAi( 2, {constructing = true, repairing = true, serfLimit = 9} )
...
function SpielmodKoop0()
	...
	KerbTicker = StartCountdown(60*60,KerbVorbereitung,true)
	StartCountdown(58*60,AngriffsErinnerung,false)
...
function SpielmodKoop2()
	Message("Ihr habt den @color:0,250,200 <<IRSINNIGEN KOOPERATIONSMODUS>> @color:255,255,255 gewählt")
	KerbTicker = StartCountdown(35*60,KerbVorbereitung,true)
	StartCountdown(43*60,ExtraTruppen, false)
	...
	for i = 3,8 do
		...
		ResearchTechnology(Technologies.T_Gunpowder, i)
...
function Share()
	...
	StartSimpleJob("ShareActivate")
end
function ShareActivate()
	if IsExisting("ShareAbfrage") then
		ActivateShareExploration(1,2,true)
		ActivateShareExploration(2,1,true)
		return true
	else
		return true
	end
...
function SetupKi2()
    MapEditor_SetupAI(2,1,Logic.WorldGetSize(),1,"Player2",3,0)
	SetupPlayerAi(2, {
	serfLimit = 12,
	extracting = 1,
	constructing = true,
...
function InitPlayerColorMapping()
	XGUIEng.SetText(""..
		...
		" @cr @cr @color:150,0,255 Ghoul @color:230,0,240 @cr (2) Aufstieg des Bösen - Kerberos dunkle Horden @cr "..    
		" @color:230,0,240 v1.7 ")
...
function KerbVorbereitung()
	...
	StartCountdown(20*60*gvDiffLVL,SpawnVorb,false)
end
...
function EliteSpawns()
	for i = 4,8 do
		if IsExisting("P"..i.."HQ") then 
			if Logic.GetNumberOfEntitiesOfTypeOfPlayer(i, Entities.CU_VeteranMajor) <= (6 / gvDiffLVL) then
				CreateEntity(3,Entities.CU_VeteranMajor,GetPosition("P"..i.."Elite"))
			end
		end
	end
	StartCountdown(10*gvDiffLVL*60,SpawnVorb,false)
	return true
end
function AngriffsErinnerung()
	...
	--
	MapEditor_SetupAI(8,2,Logic.WorldGetSize(),math.min(5-gvDiffLVL,3),"P8HQ",3,0)
	SetupPlayerAi( 8, {constructing = true, repairing = true, extracting = 1, serfLimit = 19} )
	MapEditor_SetupAI(7,2,Logic.WorldGetSize(),math.min(5-gvDiffLVL,3),"P7HQ",3,0)
	SetupPlayerAi( 7, {constructing = true, repairing = true, extracting = 1, serfLimit = 17} )
	MapEditor_SetupAI(6,2,Logic.WorldGetSize(),math.min(5-gvDiffLVL,3),"P6",3,0)
	SetupPlayerAi( 6, {constructing = true, repairing = true, extracting = 1, serfLimit = 19} )
	MapEditor_SetupAI(5,2,Logic.WorldGetSize(),math.min(5-gvDiffLVL,3),"P5",3,0)
	SetupPlayerAi( 5, {constructing = true, repairing = true, extracting = 1, serfLimit = 17} )
	MapEditor_SetupAI(4,2,Logic.WorldGetSize(),math.min(5-gvDiffLVL,3),"P4",3,0)
	SetupPlayerAi( 4, {constructing = true, repairing = true, extracting = 1, serfLimit = 19} )
	MapEditor_SetupAI(3,2,Logic.WorldGetSize(),math.min(5-gvDiffLVL,3),"P3",3,0)
	SetupPlayerAi( 3, {constructing = true, repairing = true, extracting = 1, serfLimit = 17} )
	--
	for i = 3,7 do
		MapEditor_Armies[i].offensiveArmies.strength = round(22-(4*gvDiffLVL))
	end
	MapEditor_Armies[8].offensiveArmies.strength = round(35-(5*gvDiffLVL))
	for i = 3,8 do
		ResearchAllTechnologies(i, false, false, false, true, false)
	end
	StartSimpleJob("RedirectP8_Job")
end
function RedirectP8_Job()
	if IsDead("P8HQ") then
		StartCountdown(5*60,RedirectP8,false)
		return true
	end
end
function RedirectP8()
	MapEditor_Armies[8].offensiveArmies.position = GetPosition("P9")
	MapEditor_Armies[8].defensiveArmies.position = GetPosition("P9")
end
function KerbVoice()
...
function Sound1()
	Sound.PlayGUISound(Sounds.VoicesHero7_HERO7_InflictFear_rnd_01,120)
end
function Sound2()
	Sound.PlayGUISound(Sounds.VoicesHero7_HERO7_Madness_rnd_01,120)
end
function Sound3()
	Sound.PlayGUISound(Sounds.VoicesHero7_HERO7_Attack_rnd_01,120)
end
function Sound4()
	Sound.PlayGUISound(Sounds.VoicesHero7_HERO7_InflictFear_rnd_02,120)
end
function Sound5()
	Sound.PlayGUISound(Sounds.VoicesHero7_HERO7_Madness_rnd_02,120)
end
function Sound6()
	Sound.PlayGUISound(Sounds.VoicesHero7_HERO7_InflictFear_rnd_03,120)
end
function Sound7()
	Sound.PlayGUISound(Sounds.VoicesHero7_HERO7_FunnyComment_rnd_01,120)
end
function Sound8()
	Sound.PlayGUISound(Sounds.VoicesHero7_HERO7_Attack_rnd_05,120)
	StartCountdown(4*60,KerbVoice,false)
	StartCountdown(20*60*gvDiffLVL, UpgradeKI, false)
end
function UpgradeKI()
	for i = 3,8 do
		MapEditor_Armies[i].offensiveArmies.strength = MapEditor_Armies[i].offensiveArmies.strength + round(12-(gvDiffLVL*3))
	end
	MapEditor_Armies[8].offensiveArmies.strength = MapEditor_Armies[8].offensiveArmies.strength + round(15-(gvDiffLVL*3))
end
--------------------------------------------------------------------------------------------------
