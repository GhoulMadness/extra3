...
EMS_CustomMapConfig =
{
	...
	for i = 1,2 do
		...
		Logic.AddMercenaryOffer( mercTent, Entities.CU_BlackKnight_LeaderSword3, 3, ResourceType.Gold, 750, ResourceType.Wood, 500 );
		Logic.AddMercenaryOffer( mercTent, Entities.CU_BlackKnight_LeaderMace2, 3, ResourceType.Wood, 600 );
		Logic.AddMercenaryOffer( mercTent, Entities.CU_VeteranLieutenant, 1, ResourceType.Iron, 1200, ResourceType.Sulfur, 2500);
		Logic.AddMercenaryOffer( mercTent, Entities.PV_Catapult, 2, ResourceType.Gold, 2000 );
	end
	for i = 1,4 do
	...
	Callback_OnGameStart = function()
		...
		Trigger.RequestTrigger(Events.LOGIC_EVENT_ENTITY_CREATED, "", "OnOuterPositionVCOrPileCreated", 1,{},{})
		Trigger.RequestTrigger(Events.LOGIC_EVENT_ENTITY_DESTROYED, "", "OnOuterPositionVCOrPileDestroyed", 1,{},{})
	end,
		...
		for armyTypeIndex = 1,2 do
			...
			if armyTypeIndex == 1 then
				if IsAlive("t1_"..armyPosIndex) or IsAlive("t2_"..armyPosIndex) then
					...
					if table.getn(ArmyTable[army.player][army.id + 1].IDs) < 2 then
				...
				if IsAlive("tent_"..armyPosIndex) then
					allArmySpawnsDead = false;
					if table.getn(ArmyTable[army.player][army.id + 1].IDs) < 1 then
						EnlargeArmy(army, WT.GetRandomTroopDesc());
					end
				end
			end
...
function WT.StartSuddenDeath()
	...
	DestroyEntity("ruinX");
	Logic.CreateEntity(Entities.XD_VillageCenter_Ruin, Logic.WorldGetSize()/2, Logic.WorldGetSize()/2, 0, 0);
	...
	if math.mod(WT.TowerHurtCounter, 10) == 0 then
		...
		for i = 1, table.getn(WT.DefendingTowers) do
			if IsAlive(WT.DefendingTowers[i]) then
				...
				Logic.HurtEntity(WT.DefendingTowers[i], 6);
...
WT.Mercenaries = {
	Logic.GetEntityIDByName("merc1"),
	Logic.GetEntityIDByName("merc2")
}
...
WT.Teams = {[1] = {1,2},
			[2] = {3,4}}
WT.GetTeamIDByPlayerID = function(_pID)
	for k = 1,2 do
		for v = 1,2 do
			if WT.Teams[k][v] == _pID then
				return k
			end
		end
	end
end
WT.GetPlayerIDsByTeamID = function(_tID)
	return WT.Teams[_tID]
end
WT.OuterPositionsControl = {}
-- time needed after both sides are controlled by the same team
WT.OuterPositionsControl.Treshold = 3*60
-- time needed per pulse
WT.OuterPositionsControl.BaseTimePerPulse = 30
-- random time needed per pulse
WT.OuterPositionsControl.RandomTimePerPulse = 15
-- amount of silver piles spawned per pulse
WT.OuterPositionsControl.PilesPerPulse = 2
-- maximum number of piles
WT.OuterPositionsControl.MaxPiles = 10
-- search range for pile positions
WT.OuterPositionsControl.PilesSearchRadius = 6000
-- resource amount per pile
WT.OuterPositionsControl.BaseResourceAmount = 40
-- random resource amount per pile
WT.OuterPositionsControl.RandomResourceAmount = 20
WT.OuterPositionsControl.Positions = {	{X = 70200, Y = 38500},
										{X = 6900, Y = 38400}}
WT.OuterPositionsControl.ControllingTID = {	["East"] = 7,
											["West"] = 7}
WT.OuterPositionsControl.PositionKeyByIndex = {	[1] = "East",
												[2] = "West"}
WT.OuterPositionsControl.PositionIndexByKey = {	["East"] = 1,
												["West"] = 2}
WT.OuterPositionsControl.CurrentPileAmount = {	["East"] = 0,
												["West"] = 0}
WT.OuterPositionsControl.CurrentPulse = 0
function OnOuterPositionVCOrPileCreated()

	local entityID = Event.GetEntityID()
	local etype = Logic.GetEntityType(entityID)

	if etype == Entities.PB_VillageHall1 and GetDistance(GetPosition(entityID), {X = 38400, Y = 38400}) > 3000 then
		Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND, "", "OuterPositionVCControl", 1,{},{entityID})
	end

end
function OnOuterPositionVCOrPileDestroyed()

	local entityID = Event.GetEntityID()
	local etype = Logic.GetEntityType(entityID)

	if etype == Entities.PB_VillageHall1 then
		local pos = GetPosition(entityID)
		local posname
		for i = 1,2 do
			if WT.OuterPositionsControl.Positions[i].X == pos.X and WT.OuterPositionsControl.Positions[i].Y == pos.Y then
				posname = WT.OuterPositionsControl.PositionKeyByIndex[i]
				break
			end
		end
		WT.OuterPositionsControl.ControllingTID[posname] = 7
		WT.OuterPositionsControl.CurrentPulse = 0
	elseif etype == Entities.XD_Silver1 then
		local pos = GetPosition(entityID)
		if GetDistance(pos, WT.OuterPositionsControl.Positions[1]) <= WT.OuterPositionsControl.PilesSearchRadius then
			WT.OuterPositionsControl.CurrentPileAmount.East = WT.OuterPositionsControl.CurrentPileAmount.East - 1
		elseif GetDistance(pos, WT.OuterPositionsControl.Positions[2]) <= WT.OuterPositionsControl.PilesSearchRadius then
			WT.OuterPositionsControl.CurrentPileAmount.West = WT.OuterPositionsControl.CurrentPileAmount.West - 1
		end
	end

end
function OuterPositionVCControl(_entity)

	if not Logic.IsEntityAlive(_entity) then
		return true
	end
	if Logic.IsConstructionComplete(_entity) == 1 then
		...
		local tid = WT.GetTeamIDByPlayerID(playerId)
		WT.OuterPositionsControl.ControllingTID[posname] = tid
		if WT.OuterPositionsControl.ControllingTID.East ~= 7 and WT.OuterPositionsControl.ControllingTID.East == WT.OuterPositionsControl.ControllingTID.West then
			Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND, "OuterPositionPileControl_Condition", "OuterPositionPileControl_Action", 1,{1},{})
		end
		return true
	end
end
OuterPositionPileControl_Condition = function(_flag)
	if _flag then
		return Counter.Tick2("OuterPositionPileControl_Condition_Tick", WT.OuterPositionsControl.Treshold)
	else
		return Counter.Tick2("OuterPositionPileControl_Condition_Tick", WT.OuterPositionsControl.BaseTimePerPulse + math.random(WT.OuterPositionsControl.RandomTimePerPulse))
	end
end
OuterPositionPileControl_Action = function()
	if WT.OuterPositionsControl.ControllingTID.East == 7 or WT.OuterPositionsControl.ControllingTID.West == 7 or WT.OuterPositionsControl.ControllingTID.East ~= WT.OuterPositionsControl.ControllingTID.West then
		return true
	end
	for i = 1, WT.OuterPositionsControl.PilesPerPulse do
		for k = 1, 2 do
			if WT.OuterPositionsControl.CurrentPileAmount[WT.OuterPositionsControl.PositionKeyByIndex[k]] < WT.OuterPositionsControl.MaxPiles then
				local pos = WT.OuterPositionsControl.FindPileSweetSpot(WT.OuterPositionsControl.PositionKeyByIndex[k])
				if pos then
					local id = Logic.CreateEntity(Entities.XD_Silver1, pos.X, pos.Y, 0, 0)
					Logic.SetResourceDoodadGoodAmount(id, WT.OuterPositionsControl.BaseResourceAmount + math.random(WT.OuterPositionsControl.RandomResourceAmount))
					WT.OuterPositionsControl.CurrentPileAmount[WT.OuterPositionsControl.PositionKeyByIndex[k]] = WT.OuterPositionsControl.CurrentPileAmount[WT.OuterPositionsControl.PositionKeyByIndex[k]] + 1
				end
			else
				break
			end
	...
	WT.OuterPositionsControl.SpawnBarbarianAttackers()
	WT.OuterPositionsControl.CheckForNextPulse()
	return true
end
WT.OuterPositionsControl.CheckForNextPulse = function()
	Trigger.RequestTrigger(Events.LOGIC_EVENT_EVERY_SECOND, "OuterPositionPileControl_Condition", "OuterPositionPileControl_Action", 1,{},{})
end
WT.OuterPositionsControl.FindPileSweetSpot = function(_areaname)

	local pos = {}
	local index = WT.OuterPositionsControl.PositionIndexByKey[_areaname]
	local center = WT.OuterPositionsControl.Positions[index]
	while true do
		local X = math.random(center.X - WT.OuterPositionsControl.PilesSearchRadius, center.X + WT.OuterPositionsControl.PilesSearchRadius)
		local Y = math.random(center.Y - WT.OuterPositionsControl.PilesSearchRadius, center.Y + WT.OuterPositionsControl.PilesSearchRadius)
		local height, blockingtype, sector, tempterrType = CUtil.GetTerrainInfo(X, Y)
		if sector > 0 and blockingtype == 0 and (height > CUtil.GetWaterHeight(X/100, Y/100)) then
			pos.X, pos.Y = X, Y
			break
		end
	end
...
WT.OuterPositionsControl.BarbarianSpawnPos = {East = {{X = 75400, Y = 45000}, {X = 75400, Y = 32000}}, West = {{X = 1400, Y = 45000}, {X = 1400, Y = 32000}}}
WT.OuterPositionsControl.SpawnBarbarianAttackers = function()
	WT.OuterPositionsControl.CurrentPulse = WT.OuterPositionsControl.CurrentPulse + 1
	local id1, id2, id3, id4
	for i = 1, math.max(math.min(WT.OuterPositionsControl.CurrentPulse, 5), 1) do
		for k = 1, 2 do
			id4 = CreateEntity(7, Entities.PV_Cannon2, WT.OuterPositionsControl.BarbarianSpawnPos.East[k])
			id1 = AI.Entity_CreateFormation(7, Entities.CU_BanditLeaderSword1, 0, 10, WT.OuterPositionsControl.BarbarianSpawnPos.East[k].X, WT.OuterPositionsControl.BarbarianSpawnPos.East[k].Y, 0, 0, 0, 0)
			id2 = AI.Entity_CreateFormation(7, Entities.CU_BanditLeaderBow1, 0, 10, WT.OuterPositionsControl.BarbarianSpawnPos.East[k].X, WT.OuterPositionsControl.BarbarianSpawnPos.East[k].Y, 0, 0, 0, 0)
			id3 = AI.Entity_CreateFormation(7, Entities.CU_BlackKnight_LeaderSword3, 0, 6, WT.OuterPositionsControl.BarbarianSpawnPos.East[k].X, WT.OuterPositionsControl.BarbarianSpawnPos.East[k].Y, 0, 0, 0, 0)
			Logic.GroupAttackMove(id1, WT.OuterPositionsControl.Positions[1].X - 600, WT.OuterPositionsControl.Positions[1].Y)
			Logic.GroupAttackMove(id2, WT.OuterPositionsControl.Positions[1].X - 600, WT.OuterPositionsControl.Positions[1].Y)
			Logic.GroupAttackMove(id3, WT.OuterPositionsControl.Positions[1].X - 600, WT.OuterPositionsControl.Positions[1].Y)
			Logic.GroupAttackMove(id4, WT.OuterPositionsControl.Positions[1].X - 600, WT.OuterPositionsControl.Positions[1].Y)
		end
		for j = 1, 2 do
			id4 = CreateEntity(7, Entities.PV_Cannon2, WT.OuterPositionsControl.BarbarianSpawnPos.West[j])
			id1 = AI.Entity_CreateFormation(7, Entities.CU_BanditLeaderSword1, 0, 10, WT.OuterPositionsControl.BarbarianSpawnPos.West[j].X, WT.OuterPositionsControl.BarbarianSpawnPos.West[j].Y, 0, 0, 0, 0)
			id2 = AI.Entity_CreateFormation(7, Entities.CU_BanditLeaderBow1, 0, 10, WT.OuterPositionsControl.BarbarianSpawnPos.West[j].X, WT.OuterPositionsControl.BarbarianSpawnPos.West[j].Y, 0, 0, 0, 0)
			id3 = AI.Entity_CreateFormation(7, Entities.CU_BlackKnight_LeaderSword3, 0, 6, WT.OuterPositionsControl.BarbarianSpawnPos.West[j].X, WT.OuterPositionsControl.BarbarianSpawnPos.West[j].Y, 0, 0, 0, 0)
			Logic.GroupAttackMove(id1, WT.OuterPositionsControl.Positions[2].X + 600, WT.OuterPositionsControl.Positions[2].Y)
			Logic.GroupAttackMove(id2, WT.OuterPositionsControl.Positions[2].X + 600, WT.OuterPositionsControl.Positions[2].Y)
			Logic.GroupAttackMove(id3, WT.OuterPositionsControl.Positions[2].X + 600, WT.OuterPositionsControl.Positions[2].Y)
			Logic.GroupAttackMove(id4, WT.OuterPositionsControl.Positions[2].X + 600, WT.OuterPositionsControl.Positions[2].Y)
		end
	end
end