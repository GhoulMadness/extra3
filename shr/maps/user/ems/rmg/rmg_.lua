...
--Script.Load( RMGPath .. "\\structuresets.lua" )
--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++--
...
function RMG.GetPlayerConfigByGDBData()
	local playert = {{Id = 1, Team = 1, IsHuman = true}}
	local teams = {1}
	...
	for i = 2, 8 do
		if GDB.GetValue("Singleplayer\\RandomMapData\\MapAI_" .. i .. "_Active") == 2 then
			...
			table.insert(playert, {Id = i, Team = team, IsHuman = false})
...
function RMG.InitGenerationData()
	...
	RMG.GenerationData.GateLayout				= 1
	RMG.GenerationData.GateSize					= 1
	RMG.GenerationData.GenerateRoads 			= true

	RMG.GenerationData.LandscapeSetKey			= RMG.LandscapeSetKeys[GDB.GetValue("Singleplayer\\RandomMapData\\MapSetting")].id
	RMG.GenerationData.MirrorMap				= Num2Bool(GDB.GetValue("Singleplayer\\RandomMapData\\MapMirror"))
	RMG.GenerationData.RandomPlayerPosition		= false
	...
	local contentClayPit	= 50000
	local contentClayPile	= 1500
	local contentStonePit	= 50000
	local contentStonePile	= 1000
	local contentIronPit	= 30000
	local contentIronPile	= 1000
	local contentSulfurPit	= 30000
	local contentSulfurPile	= 1000
	local contentGoldPit	= 50000
	local contentSilverPit	= 1500
	local contentWoodPile	= 30000
	...
	local nplayers, players, nteams = RMG.GetPlayerConfigByGDBData()
...
function RMG.FillPlayerStruct()
	...
	local amountClayPit		= RMG.ResPitValByTypeAndEnum["Normal"][EvalMapResources]
	local amountClayPile	= RMG.ResPileValByTypeAndEnum["Normal"][EvalMapResources]
	local amountStonePit	= RMG.ResPitValByTypeAndEnum["Normal"][EvalMapResources]
	local amountStonePile	= RMG.ResPileValByTypeAndEnum["Normal"][EvalMapResources]
	local amountIronPit		= RMG.ResPitValByTypeAndEnum["Combat"][EvalMapResources]
	local amountIronPile	= RMG.ResPileValByTypeAndEnum["Combat"][EvalMapResources]
	local amountSulfurPit	= RMG.ResPitValByTypeAndEnum["Combat"][EvalMapResources]
	local amountSulfurPile	= RMG.ResPileValByTypeAndEnum["Combat"][EvalMapResources]
	local amountGoldPit		= RMG.ResPitValByTypeAndEnum["Combat"][EvalMapResources]
	local amountSilverPit	= RMG.ResPitValByTypeAndEnum["Rare"][EvalMapResources]
	local amountWoodPile	= 0
	local amountVillageCenter = RMG.StructureValByTypeAndEnum["VillageCenter"][EvalMapVC]
	local amountVillageHall	= RMG.StructureValByTypeAndEnum["VillageHall"][EvalMapVC]
	local amountLighthouse	= RMG.StructureValByTypeAndEnum["Lighthouse"][EvalMapVC]
...
function RMG.CreateFences( _generationdata )
	...
	for delta = 0, math.rad( 360 ), a do
		...
		if delta > c + d and delta < b - d then
			...
			if Peacetime and Peacetime > 0 then
...
function RMG.CreateEntities( _generationdata )
	...
	for id in CEntityIterator.Iterator( CEntityIterator.OfTypeFilter( Entities.XD_ScriptEntity ) ) do
		...
		if name == "woodpile" then
			...
			CreateWoodPile( "woodpile"..woodpilecounter, 99999 )
...
function RMG.DeleteBridges( _generationdata )
	if Peacetime and Peacetime > 0 and _generationdata.TeamBorderType == 3 then
...
function RMG.PlayerFeedbackReady( _name, _playerindex )
	...
	-- host: start game
	if GUI.GetPlayerID() == 1 or not CNetwork then
		RMG.EMS_GL_StartRequestYes()
	end
end
--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++--
...
RMG.EMS_GL_StartRequestYes = function()
...
--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++--
 ...
 --++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++--
--[[
Script.Load(RMGPath .. "\\rmg_guil.lua")
Script.Load(RMGPath .. "\\rmg_ruledata.lua")
